...
const isObject3DRef = (
  ref: unknown
): ref is React.MutableRefObject<THREE.Object3D> =>
  typeof ref === "object" &&
  ref !== null &&
  "current" in ref &&
  ...
  (
    ...
    React.useLayoutEffect(() => {
      ...
      const _curves =
        ...
        curves.length > 0 ? curves : (ref.current as any)?.__r3f.objects;
    ...
    React.useImperativeHandle<THREE.Group, THREE.Group>(
      fref,
      () => ref.current as THREE.Group,
      []
    );
    ...
    useFrame((_state, delta) => {
      ...
      if (path.getCurveLengths().length > 0) {
        ...
        path.getPointAt(
          misc.repeat(pos.current - (last - state.offset), 1),
          state.next
        );
        const target =
